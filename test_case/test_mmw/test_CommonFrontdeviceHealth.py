...
sheet_name = eval(HandleConfig().get_value('commonFrontdeviceHealth.ini', 'excel', 'sheet_name_devicehealth'))
file_name = os.path.join(testData_dir, file_name)
MaintenanceManagmentWeb_data = HandleExcel(file_name, sheet_name).read()
...
class Test_MaintenanceManagmentWeb(unittest.TestCase):
    ...
    @data(*MaintenanceManagmentWeb_data)
