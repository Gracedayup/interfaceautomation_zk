...
sheet_name = eval(HandleConfig().get_value('basicdata.ini', 'excel', 'commonSpotting_sheet_name'))
file_name = os.path.join(testData_dir, file_name)
commonSpotting_data = HandleExcel(file_name, sheet_name).read()
...
class Basic_Data(unittest.TestCase):
        ...
        @data(*commonSpotting_data)
        def test_commonSpotting(self,test_data):
            # 拼接接口url
