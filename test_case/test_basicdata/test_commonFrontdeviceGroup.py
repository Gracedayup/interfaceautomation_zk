...
# -*- coding: utf-8 -*-
"""
    Time     : 2020/4/14 10:16
    Author   : yanw
    File       : test_commonFrontdeviceGroup.py
    Software: PyCharm
    基础数据-设备管理-设备分组
    """
...
sheet_name = eval(HandleConfig().get_value('basicdata.ini', 'excel', 'commonFrontdeviceGroup_sheet_name'))
file_name = os.path.join(testData_dir, file_name)
commonFrontdeviceGroup_data = HandleExcel(file_name, sheet_name).read()
...
class Basic_Data(unittest.TestCase):
        ...
        @data(*commonFrontdeviceGroup_data)
        def test_commonFrontdeviceGroup(self,test_data):
            # 拼接接口url
