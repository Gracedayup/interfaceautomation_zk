...
"""
    ...
    Author   : yanw
    File       : test_commonAreaConfig.py
    Software: PyCharm
     基础数据-区域管理-数据分析进出区域地图规划信息
    """
...
sheet_name = eval(HandleConfig().get_value('basicdata.ini', 'excel', 'commonAreaConfig_sheet_name'))
file_name = os.path.join(testData_dir, file_name)
commonAreaConfig_data = HandleExcel(file_name, sheet_name).read()
...
class Basic_Data(unittest.TestCase):
        ...
        @data(*commonAreaConfig_data)
        def test_commonAreaConfig(self,test_data):
            # 拼接接口url
