...
# -*- coding: utf-8 -*-
"""
    Time     : 2020/4/16 10:21
    Author   : yanw
    File       : test_commonStaff.py
    Software: PyCharm
    基础数据-人员管理-人员
    """
...
sheet_name = eval(HandleConfig().get_value('basicdata.ini', 'excel', 'commonStaff_sheet_name'))
file_name = os.path.join(testData_dir, file_name)
commonStaff_data = HandleExcel(file_name, sheet_name).read()
...
class Basic_Data(unittest.TestCase):
        ...
        @data(*commonStaff_data)
        def test_commonStaff(self,test_data):
            # 拼接接口url
