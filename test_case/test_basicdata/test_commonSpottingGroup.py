...
"""
    ...
    Author   : yanw
    File       : test_commonSpottingGroup.py
    Software: PyCharm
    基础数据-路口管理-路口组
    """
...
sheet_name = eval(HandleConfig().get_value('basicdata.ini', 'excel', 'commonSpottingGroup_sheet_name'))
file_name = os.path.join(testData_dir, file_name)
commonSpottingGroup_data = HandleExcel(file_name, sheet_name).read()
...
class Basic_Data(unittest.TestCase):
        ...
        @data(*commonSpottingGroup_data)
        def test_commonSpottingGroup(self,test_data):
            # 拼接接口url
