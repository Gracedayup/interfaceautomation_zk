...
# -*- coding: utf-8 -*-
"""
    Time     : 2020/4/15 16:37
    Author   : yanw
    File       : test_commonCompany.py
    Software: PyCharm
    基础数据-单位管理-单位管理
    """
...
file_name = eval(HandleConfig().get_value('smw.ini', 'excel', 'file_name'))
sheet_name = eval(HandleConfig().get_value('smw.ini', 'excel', 'account'))
file_name = os.path.join(testData_dir, file_name)
cases = HandleExcel(file_name, sheet_name).read()


@ddt
class Account_Data(unittest.TestCase):
    ...
    @data(*cases)
    def test_commonCompany(self, test_data):
        # 拼接接口url
            ...
            if str(test_data["is_replace"]).lower() == 'true':
                ...
                if (type(param).__name__ == 'list'):
                    for data_dic in param:
                        for key in data_dic:
                            if key == 'companyid':
                                data_dic[key] = data[0]['COMPANYID']

                elif (type(param).__name__ == 'dict'):
                    for key in param:
                        if key == 'companyid':
                            param[key] = data[0]['COMPANYID']

                if str(test_data['method']).lower() == 'delete' or str(test_data['method']).lower() == 'post':
                    if str(test_data["is_json"]).lower() == 'true':
                        pass
                    else:
                        param = param['companyid'].split()
